/**
 * monthly-report.js — Monthly price report interactive enhancement
...
// ---- Column definitions ----
const COLUMNS = [
  { key: 'date', labelKey: 'reports.col.date', labelFallback: 'Date', sortable: true, filterable: true },
  { key: 'plan', labelKey: 'reports.col.plan', labelFallback: 'Plan', sortable: true, filterable: true },
  { key: 'field', labelKey: 'reports.col.field', labelFallback: 'Price field', sortable: true, filterable: true },
  { key: 'price', labelKey: 'reports.col.price', labelFallback: 'Price', sortable: true, filterable: true, numeric: true },
  { key: 'change', labelKey: 'reports.col.change', labelFallback: 'Change', sortable: true, filterable: true }
];
...
function renderTable(wrap, allRows) {
  ...
  tbody.innerHTML = filtered.map(row => {
    // Preserve original HTML (with links/badges) from the static table
  ...
  if (countEl) {
    const labelEn = `Showing ${filtered.length} of ${allRows.length} records`;
    const labelZh = `显示 ${filtered.length} / ${allRows.length} 条记录`;
...
function renderHeaderCell(col, allRows) {
  ...
  return `
    <th scope="col" class="plan-column-filter px-3 py-3 text-left font-semibold text-slate-900 dark:text-white">
      <button type="button" class="plan-column-filter-trigger${isActive || hasFilter ? ' is-active' : ''}" data-report-sort="${col.key}" title="${tooltipText} ${label}">
